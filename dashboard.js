...
function normalizeDisplayName(name) {
  let clean = String(name || '未命名节点')
    .replace(/[\u{1F1E6}-\u{1F1FF}]{2}/gu, '')
    .replace(/^\s*[-|｜·•:：_]+\s*/, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
  const relay = clean.match(/^(.+?)[-—–_\s]+(中转|中轉|前置|反连|反連)[-—–_\s]+(.+)$/);
  if (relay && relay[1] && relay[3]) clean = `${relay[3].trim()}(${relay[1].trim()}${relay[2]})`;
  return clean || '未命名节点';
}
...
function displayName(name){var clean=String(name||"未命名节点").replace(/[\u{1F1E6}-\u{1F1FF}]{2}/gu,"").replace(/^\\s*[-|｜·•:：_]+\\s*/,"").replace(/\\s{2,}/g," ").trim();var relay=clean.match(/^(.+?)[-—–_\\s]+(中转|中轉|前置|反连|反連)[-—–_\\s]+(.+)$/);if(relay&&relay[1]&&relay[3])clean=relay[3].trim()+"("+relay[1].trim()+relay[2]+")";return clean||"未命名节点";}
